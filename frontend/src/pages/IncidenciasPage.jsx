...
import { Plus, Pencil, CheckCircle, X } from 'lucide-react';
...
const IncidenciasPage = () => {
  ...
  const load = useCallback(() => {
    ...
    api.get('/incidencias', { params }).then((r) => setData(r.data.data));
  }, [page, estadoFilter]);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      toastError('Error al actualizar incidencia');
    } finally {
      setSaving(false);
    }
  };
            ...
            {data.data?.length === 0 ? (
              ...
              data.data?.map((inc) => (
                <tr key={inc.id_incidencia} className="border-t border-slate-50 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 font-mono text-slate-600">{inc.envio?.codigo_envio || '—'}</td>
